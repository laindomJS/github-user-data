...
{#if $user.name === undefined}
	<h3 class="font-bold text-lg md:text-lg text-slate-300 ml-2 md:ml-0">No results yet...</h3>
{:else}
	<article class="w-full bg-[#1f2a48] rounded-lg p-4 md:p-8">
		
    <div class="flex flex-col md:flex-row md:justify-between md:items-center">

			<div class="pl-2 md:pl-0">
				<img
					class="bg-white rounded-full w-24 h-24"
					src={$user.avatar_url}
					alt={$user.name}
				/>
				<div class="mt-2">
					...
					<span class="text-slate-300">Joined at: {dayjs($user.created_at).format('YYYY MMM DD')}</span>
					<div class="mt-2"> <span> <LocationIcon /> {$user.location}</span></div>
					<p class="mt-4 w-64 text-sm">{$user.bio}</p>
				</div>
			</div>

			<div class="bg-[#141c2f] w-full p-4 flex flex-col md:flex-row space-y-6 md:space-y-0 justify-between rounded-lg mt-4">
